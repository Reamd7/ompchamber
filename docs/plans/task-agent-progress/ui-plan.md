...
> 状态：**层 1 与批次 A/B/C 完成；registry 分脑已修；钻入已重构为"真·只读子会话视图"（方案 B）；diskScan 已接入（2026-09-05）**——重启后历史运行行可点、可钻入。链路：list 端点 `ensureDirectory(directory)` → 引擎一次性扫描该目录 sessions 根下的嵌套转录（`<ts>_<hostID>/<task>.jsonl`）→ `DiskScanRow{childSessionID, file(内部)}` 缓存 → 聚合器合并为 historical 行（registry 行优先）；`#findSubagentSessionFile` 在 registry 未命中后回退磁盘缓存，`getSession`/`getMessagesPage` 因此对重启后的历史子会话同样 200。开发栈实测（用户旧会话 `01a070c3`，引擎重启、无 registry ref）：行携带 `childSessionID=01a070db…`，点 Open run 后 iframe readOnly 渲染 BranchScout 完整任务对话（markdown Target/Change/Acceptance）。**仍开放**：WS→store 应用段的异步卡片 DOM 半步（transport 已证到达）。
>
...
>
>
> **浏览器目验记录（2026-09-04，dev 服务器 + 真实会话）**：新建 bug-fix 会话并令模型经 task 工具派 scout 子代理。已证实：① 任务卡片行渲染完整（状态点/agent 名/任务描述/`0 · 0ms` 用量位/Open run 按钮）；② C1 跳转全链路——点 Open run 打开 agentRun 侧板标签页，轨道 surface 以 has-content 语义现身并激活；③ transcript 端点路由与错误语义正确（无 registry ref → 404 `agent-run-not-found`）；④ 空态文案正确渲染。**追查修正（2026-09-04 晚，推翻同日早前的"spawn 挂起/onProgress 链中断"假设）**：⑴ 真 bug 之一——`omp-host` 从未把 registry 事件接到 `aggregator.refresh()`（已修，`055d107c`）；⑵ 之后所有"子代理秒挂"复现均为**诊断插桩自己损坏了 vendored SDK**（先误删 `monitor.attach(session)` 行，缓存硬链接又让重装带回损坏态、再丢 `const sessionManager = await …` 行）——逐处修复后结构复原、375/0 全绿；⑶ 修复态实测：子代理运行到完成（JSONL 44→85 行、产物落盘），但两轮派发均走**异步 job 路径**（bundled agent 无 `blocking: true` frontmatter，`blocking = agent.blocking === true` 恒 false）——task 工具调用在 spawn 时即 completed、进度快照冻结在 Pending/0ms 是该路径的 SDK 语义（进度走 jobs 通道而非 tool_execution_update），引擎→wire→卡片的重复更新投影链有 dispositions 测试背书、对同步派发成立。**⑷ 第二个真 bug（2026-09-04 人模测试发现，待修）**：registry 分脑——`createAgentSession` 把会话自身 ref 注册进传入的 per-session registry，但 task executor 全部走 `AgentRegistry.global()`（executor.ts 282/1267/2349/2457/2606）注册子代理 ref；`055d107c` 订阅的是 per-session registry，故 revision 停 1、rows 恒空。正解：engine 启动时订阅 `AgentRegistry.global().onChange` → refresh，`agentsSnapshot` 需把 global registry 的子代理 ref 映射回会话/目录（sessionFile 路径含 `<ts>_<sessionID>/` 可解析），diskScan 未接线（运行中子代理 JSONL 在盘、扫描可发现）。人模测试另证实运行态面板/徽章/行因 ⑷ 仍不可见，异步派发下卡片冻结属语义而非缺陷。
