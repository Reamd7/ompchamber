...
**追加(08-23,用户两点要求)**:①可编辑 agent 的「在文件管理器中打开」——镜像 plugins.v1 的 reveal:服务端 `POST /api/omp/agent-definitions/{name}/reveal`(bundled→409、未管理/未知→404,opener 复用 `domain-plugins.js revealCommand`,不造第二份),UI 在 Source 行加 outline 按钮(仅 project/user 且有 filePath 时渲染);②info 提示挂官方文档——`SettingsInfoHint` 扩展 `docsUrl`(tooltip 底部外链,新 tab),五个原语(`SettingsSection/FieldRow/StackedField/CheckboxRow/ControlGroup`)透传 `infoDocsUrl`;agents 表单 7 个字段 + Task overrides 区映射到 oh-my-pi 官方 docs(task-agent-discovery.md 锚点 / prewalk.md / advisor-watchdog.md / agent-hub.md,URL 均经 curl 验证)。验证:服务端 reveal 三态真机 E2E(200/409/404,`open -R` 真实执行);domain-modes 270/270(+reveal 用例);ui tsc 0 error;lint 与基线零差异;omp.test 47/47;i18n ×12(infoHintDocs/reveal/revealFailed)。未验证:按钮与 tooltip 链接的视觉渲染(浏览器工具会话僵死,无 CDP 可用)——链路为静态 prop 透传,tsc 已覆盖契约。
