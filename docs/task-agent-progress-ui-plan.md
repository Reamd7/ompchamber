...
>
>
> **浏览器目验记录（2026-09-04，dev 服务器 + 真实会话）**：新建 bug-fix 会话并令模型经 task 工具派 scout 子代理。已证实：① 任务卡片行渲染完整（状态点/agent 名/任务描述/`0 · 0ms` 用量位/Open run 按钮）；② C1 跳转全链路——点 Open run 打开 agentRun 侧板标签页，轨道 surface 以 has-content 语义现身并激活；③ transcript 端点路由与错误语义正确（无 registry ref → 404 `agent-run-not-found`）；④ 空态文案正确渲染。**目验追查发现真正根因（推翻"spawn 挂起"初判）**：子代理进程其实全程正常运行（会话 JSONL 持续增长、产物分页计数中），坏的是服务端两处——⑴ `omp-host` 从未把 per-session `AgentRegistry.onChange` 接到 `aggregator.refresh()`：快照 revision 恒 0、`omp.agents.updated` 从不发布，hub 行/徽章/transcript 行解析全部看到空世界（本日已修，见 engine.ts 接线）；⑵ task 卡片停在首个进度快照（Pending/0·0ms）不随运行刷新——`tool_execution_update` 只到达一次，后续 `onProgress → onUpdate` 链路在引擎进程内中断，待查（插桩实验因 dev 栈引擎进程不随 nodemon 重启而未能捕获追踪输出）。
