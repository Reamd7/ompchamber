...
- **Chat:** the model menu's role list can no longer push the search box and model list out of reach — with many configured roles the role section overflowed the menu's fixed height and everything below it (including the scrollbar) was silently clipped. The role list now scrolls within its own capped section.
