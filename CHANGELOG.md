...
- Mobile/iOS: with a hardware keyboard attached (such as an iPad Magic Keyboard), focusing the input no longer leaves a dead strip below the app that the page still scrolls under with a stray scrollbar — the app now sizes itself to the visible area above the minimized keyboard widget, in the browser, the home-screen web app, and the native iOS app.
- **Chat:** picking a model in the composer now actually switches the session's model — under the omp engine a pick only updated the app's own display (prompts stopped carrying a model when model roles shipped), so the conversation kept running on the previous model and the label snapped back after the next message. Picks now apply to the session like the terminal's `/switch`, including role chips, the model chosen before starting a new chat, multi-run launches, and todo-send/fork execution; a failed switch rolls the display back and shows an error.
- **Chat:** the model picker's role chips now follow engine settings immediately — assignments edited in Settings → Engine (or made through "Set as role" in the chat) kept showing stale models in the picker until the page was remounted. Every settings write now live-refreshes the chips over the event stream, and a failed refresh keeps the previous list instead of blanking it.
- **Chat:** the model menu's role list can no longer push the search box and model list out of reach — with many configured roles the role section overflowed the menu's fixed height and everything below it (including the scrollbar) was silently clipped. Role rows now scroll together with the model list in the menu's single scroll area.
- **Chat:** the thinking-level picker now works before a chat starts — on a fresh session it was simply absent (there was no session to set the level on), and a role chip's thinking level was ignored too. The draft picker offers the picked model's levels and applies them to the session when the first message creates it; picking a role now carries that role's explicit thinking level the same way the terminal client does. In existing sessions the chip also no longer disappears while the session's model answer is still loading — the displayed model stands in until the server confirms.
- **Chat:** history now records the exact model and thinking level each message was sent with — the engine stamps the send-time snapshot onto the user message (the effective thinking level rides the model's variant slot), and reloading a session folds the transcript's model/thinking change log so older messages carry the state they actually ran with instead of the session's current model (or none at all).
- **Chat:** model and mode switches now appear in the timeline as slim dividers — "Switched model · axonhub/gpt-5.6-luna (temporary)" and mode changes render at their point in the history (reload included), so you can see what ran where instead of reconstructing it from message metadata.
- **Chat:** the "superseded" dimming on retried turns survives a reload — the marker was rebuilt only from the live retry event, so after refreshing the failed attempt looked like a normal reply. The durable retry-end event now restores it alongside its recovery notes.
...
- **Settings:** the Agents page now manages omp's real agent definitions — your `~/.omp` worker agents and the built-in task/sonic/scout/reviewer family appear grouped by project/user/bundled scope, and edits write the same agent markdown files the terminal client uses, with omp's own fields (model patterns, thinking level, spawns, prewalk/advisor) plus per-agent task overrides. Built-in agents are read-only with a duplicate-to-user path. Previously created OpenChamber agents migrate once into those files so the engine's `task` tool can dispatch them, and the composer's persona selector now actually applies the selected persona. Agent fields with official documentation carry a link to the oh-my-pi docs inside their info tooltips, and editable definitions gain an "Open in file manager" action next to their source.
- **Engine:** the settings page now edits your omp configuration directly, with per-project model-role overrides where omp supports them; behavior instructions edit omp's own user-level AGENTS.md (legacy OpenCode files shown read-only), and MCP stays read-only with mutating controls disabled.
- **Engine:** transcript history now includes everything the engine recorded — shell and python `!`/`$` runs, file mentions, advisor notes, and extension messages render as cards instead of silently missing, and long tool runs stream partial output while they run.
- **Engine:** the sidebar gains a live agent-runs view of engine-spawned subagents and session trees show forks and their lineage.
- **Chat:** the composer's model menu now follows omp model roles — quick role chips (with each role's resolved model and thinking level), a "Set as role" action on every model row, a per-session thinking-level selector, and role assignment from the chat instead of settings spelunking. Model picks made in chat are session-scoped; new chats follow your default role. When omp's `enabledModels` is configured the picker only offers those models and warns when the current one falls outside the list.
- **Chat:** assistant messages now show the model that actually answered, and a warning marker when a retry fallback moved the turn to a backup model; context compaction shows an inline "compacting context…" state while it runs and queued messages wait for it to finish.
- **Settings:** a new Engine Settings page edits the omp engine's own configuration — every schema key with typed controls, masked credential fields, per-key validation errors, and an Approvals section for ask/timeout/tool-approval behavior. The Defaults page becomes a model-roles editor (per-role models, default thinking level, role storage), and a one-time banner offers importing an old OpenChamber default model as the default role — never overwriting a role you already configured.
- **Chat:** `/troubleshoot` replaces the guided-debug `/debug` command when the omp engine owns that name, with a one-time notice; engine commands now list alongside app commands in the slash menu with a badge when an engine command overrides an app one.
- Chat: `/tree` opens a session fork-tree view; picking a branch loads that branch's timeline, `/undo` rewinds to before your last message and puts it back in the composer, and `/redo` steps forward again.
...
- **Chat:** sessions started from the app now generate titles again — the title trigger was gated on first-prompt bookkeeping that session creation had already written, so web-created sessions never attempted one while TUI-created sessions did. Titles now follow the TUI's semantics: attempted while the first turn runs, retried on later messages, and shown live in the sidebar.
- **Chat:** sending while the agent is already working no longer fails — prompts now steer the running turn (or queue for the next one when the client asks), matching the terminal client's semantics, and `/`-commands keep working mid-turn.
- **Chat:** new sessions now use your configured default model from agent settings instead of whichever provider model happened to sort first.
- **Chat:** fixed sent messages and replies sometimes rendering twice — live streaming and reloads derived different message ids for the same message, so a re-fetch added a second copy next to the one already on screen. Both paths now resolve the same id for the same message.
...
- **Engine:** models now list from the providers omp is logged into, and skills and MCP servers load from your omp configuration and `.mcp.json`.
