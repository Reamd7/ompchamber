...
const UPDATE_CHECK_URL = process.env.OMPCHAMBER_UPDATE_API_URL || 'https://api.openchamber.dev/v1/update/check';
const GITHUB_BACKEND_DISABLED_ERROR = 'OMPChamber VS Code backend GitHub integration is disabled. Use native VS Code GitHub integrations.';
    ...
    const permissionAutoAcceptResponse = await handlePermissionAutoAcceptBridgeMessage(
      ...
      ctx?.context,
      {
        broadcast: (snapshot) => vscode.commands.executeCommand(
          'ompchamber.internal.permissionAutoAcceptSynced',
          snapshot,
        ),
      },
    );
