...
- Chat: picking a model in the composer now actually switches the session's model — under the omp engine a pick only updated the display while the conversation kept running on the previous model. Picks apply to the session like the terminal's `/switch`, including role chips, the model chosen before starting a new chat, and multi-run launches; a failed switch rolls the display back and shows an error.
- Chat: the model picker's role chips now follow engine settings immediately — assignments edited in the engine settings page (or made through "Set as role" in the chat) kept showing stale models in the picker until the webview reloaded. Every settings write now live-refreshes the chips over the event stream.
- Chat: the thinking-level picker now works before a chat starts — the draft picker offers the picked model's levels and applies them to the session when the first message creates it, and picking a role carries that role's explicit thinking level.
...
- Sessions: forking a conversation works again — a forked session opened read-only with "Subagent sessions cannot be prompted" and never appeared in the session switcher, because every fork was read as a subagent's session. Forks are normal, promptable sessions again (existing forks recover on load).
- Chat: "Fork from this message" now forks from that message — the engine ignored the clicked message and copied the whole conversation, so the fork still contained every later message. The fork now ends just before the message you branched from.
