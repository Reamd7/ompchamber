...
function createLivePortInstance(port, info, host) {
  ...
  return {
    ...
    pidFilePath: path.join(getRunDir(), `ompchamber-${port}.pid`),
    instanceFilePath: path.join(getRunDir(), `ompchamber-${port}.json`),
    mtime: 0,
    startedAt: 0,
    launchMode: 'daemon',
...
async function resolveDoctorPortStatuses(options = {}) {
  ...
  if (options.explicitPort) {
    ...
    if (desktopEntry && desktopEntry.port === requestedPort) {
      statuses.push({
        ...
        detail: 'Use a CLI instance port from `ompchamber serve` for tunneling.',
      });
    ...
    statuses.push({
      ...
      detail: `Start one with \`ompchamber serve --port ${requestedPort}\`.`,
    });
    return { statuses, availableEntries: [] };
  }
  ...
  if (runningEntries.length === 0) {
    statuses.push({
      ...
      detail: 'Start one with `ompchamber serve`.',
    });
  }
...
async function discoverRunningInstances(options = {}) {
  ...
  try {
    ...
    for (const file of pidFiles) {
      const port = parseInt(file.replace('ompchamber-', '').replace('.pid', ''), 10);
      if (!Number.isFinite(port) || port <= 0) continue;
      const pidFilePath = path.join(runDir, file);
      const pid = readPidFile(pidFilePath);
      if (!pid) {
        removePidFile(pidFilePath);
        removeInstanceFile(path.join(runDir, `ompchamber-${port}.json`));
        continue;
      }

      const instanceFilePath = path.join(runDir, `ompchamber-${port}.json`);
...
async function discoverOMPChamberInstanceOnPort(port, options = {}) {
...
async function discoverLifecycleInstances(options = {}, deps = {}) {
  ...
  const liveInstance = await discoverOMPChamberInstanceOnPort(options.port, {
    ...deps,
    host: options.host,
    runningInstances,
  });
