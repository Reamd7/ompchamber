  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (name) {
      ...
      case 'd':
        ...
        break;
      case 'try-cf-tunnel':
        removedFlagErrors.push('`--try-cf-tunnel` was removed. Use: ompchamber tunnel start --provider cloudflare --mode quick');
        break;
      case 'tunnel-qr':
        removedFlagErrors.push('`--tunnel-qr` was removed. Use: ompchamber tunnel start ... --qr');
        break;
      ...
      case 'tunnel':
        removedFlagErrors.push(`\`--${name}\` was removed from top-level serve flow. Use: ompchamber tunnel start ...`);
        break;
      default:
...
COMMANDS:
  ...
  session        Create, inspect, and read OMPChamber sessions
  models         Show default and favorite models
  projects       Show configured projects and IDs
  control        Show OMPChamber control-plane commands
...
EXAMPLES:
  ...
  ompchamber --port 8080        # Start on port 8080 (daemon)
  ompchamber --lan --port 3002  # Start on LAN at 0.0.0.0:3002
  ompchamber serve --foreground # Start in foreground (for systemd Type=simple)
  ompchamber connect-url --port 3000 --qr
  ompchamber connect-url --server https://ompchamber.example.com
  ompchamber control           # Show control-plane commands for agents/scripts
  ompchamber startup enable     # Start OMPChamber at user login
  ompchamber tunnel help        # Show tunnel lifecycle help
  ompchamber logs               # Follow logs for latest running instance
`);
}
...
USAGE:
  ompchamber tunnel <SUBCOMMAND> [OPTIONS]
...
COMMON OPTIONS:
  -p, --port              Target OMPChamber instance port
...
BEHAVIOR NOTES:
  - One active tunnel per OMPChamber instance.
...
PROFILE USAGE:
  ompchamber tunnel profile list [--provider <id>] [--json]
  ompchamber tunnel profile show --name <name> [--provider <id>] [--json]
  ompchamber tunnel profile add --provider <id> --mode managed-remote --name <name> --hostname <host> --token <token> [--force] [--json]
  ompchamber tunnel profile add --provider <id> --mode managed-remote --name <name> --hostname <host> --token-file <path> [--force] [--json]
  ompchamber tunnel profile remove --name <name> [--provider <id>] [--json]

SHELL COMPLETION:
  ompchamber tunnel completion bash   Generate Bash completion script
  ompchamber tunnel completion zsh    Generate Zsh completion script
  ompchamber tunnel completion fish   Generate Fish completion script
...
EXAMPLES:
  ...
  ompchamber tunnel start --dry-run --provider cloudflare --mode managed-remote --token-file ~/.secrets/cf-token --hostname app.example.com
  echo "$TOKEN" | ompchamber tunnel profile add --provider cloudflare --mode managed-remote --name prod-main --hostname app.example.com --token-stdin
  ompchamber tunnel profile list --provider cloudflare
  ompchamber tunnel profile list --json --show-secrets
...
function generateCompletionScript(shell) {
  ...
  if (normalized === 'bash') {
    return `# Bash completion for ompchamber tunnel
...
`;
  ...
  if (normalized === 'fish') {
    return `# Fish completion for ompchamber tunnel
