...
function getStartupServicePaths() {
  ...
  if (process.platform === 'linux') {
    return {
      platform: 'linux',
      servicePath: path.join(os.homedir(), '.config', 'systemd', 'user', 'ompchamber.service'),
    };
  }
...
function getMacosStartupWrapperPath() {
  return path.join(getDataDir(), 'bin', 'OMPChamber');
}
...
function buildMacosLaunchAgent(options = {}) {
  ...
  const env = collectStartupEnv(options);
  const logDir = path.join(os.homedir(), 'Library', 'Logs', 'OMPChamber');
...
function getStartupStatus() {
  ...
  if (paths.platform === 'linux') {
    const enabledResult = runStartupCommand('systemctl', ['--user', 'is-enabled', 'ompchamber.service'], { allowFailure: true });
    const activeResult = runStartupCommand('systemctl', ['--user', 'is-active', 'ompchamber.service'], { allowFailure: true });
...
function enableStartupService(options = {}) {
  ...
  if (paths.platform === 'macos') {
    ...
    fs.mkdirSync(path.join(os.homedir(), 'Library', 'Logs', 'OMPChamber'), { recursive: true, mode: 0o700 });
  ...
  if (paths.platform === 'linux') {
    ...
    runStartupCommand('systemctl', ['--user', 'enable', '--now', 'ompchamber.service']);
    return getStartupStatus();
  }
...
function disableStartupService() {
  ...
  if (paths.platform === 'linux') {
    runStartupCommand('systemctl', ['--user', 'disable', '--now', 'ompchamber.service'], { allowFailure: true });
    try { fs.unlinkSync(paths.servicePath); } catch {}
