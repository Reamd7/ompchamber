...
import { runtimeFetch } from '@ompchamber/ui/lib/runtime-fetch';
...
function parseIssue(payload: LinearIssue | null | undefined): LinearIssue | null {
  ...
  const comments = Array.isArray(payload?.comments)
    ? payload.comments.map(parseComment).filter((comment: LinearIssueComment | null): comment is LinearIssueComment => comment != null)
    : [];
