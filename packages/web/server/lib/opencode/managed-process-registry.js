...
//   2. Before killing, it re-verifies the live pid is still a managed engine
//      `serve` matching the recorded port (guards against the OS recycling a
//      dead pid onto an unrelated process).
...
export const createManagedProcessRegistry = ({ fs = fsp, execFileAsync = defaultExecFileAsync } = {}) => {
  ...
  const processEntry = async (entry, { log }) => {
    ...
    if (process.platform === 'win32') {
      ...
      if (windowsImageLooksLikeEngine(await readWindowsImageName(entry.pid)) && ownerGone) {
