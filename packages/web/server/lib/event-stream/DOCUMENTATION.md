...
- `parseSseEventEnvelope(block)`: parses an SSE block into `{ eventId, eventName, directory, payload }`. Data-less blocks carrying an `event:` name or `id:` are control frames (payload null) — never dropped, since resync/restart controls ride them; a block whose `data:` fails JSON parsing returns the same shape plus `malformed: true` so the reader must not advance its cursor; comment-only blocks return null.
...
- `createUpstreamSseReader(...)`: creates a start/stop reader for OpenCode SSE streams. The reader parses SSE blocks, tracks the latest `Last-Event-ID`, reconnects after closed or stalled upstream streams, and reports events through callbacks. It forwards data-less control frames (event name + id, payload null), advances its cursor on them, refuses cursor advancement for `malformed` blocks (counted via `getStats().droppedBlocks`), and caps single blocks (`maxBlockBytes`, default 16 MiB): a complete over-budget block with an `id:` line disavows itself by advancing the cursor and emitting a synthesized `omp.stream.resync` control, while an unfinished or id-less over-budget block still aborts the attempt. It learns the upstream boot identity from the `x-omp-epoch` response header, clears the cursor on a changed epoch (a stale id must never pair with the new boot), echoes the epoch on reconnects, and reports changes through `onEpochChange`; `getEpoch()` exposes the learned value. `getStats()` reports dropped blocks/bytes and epoch changes.
...
- When the shared global upstream reconnects after it was previously ready, the global WS bridge sends a fresh `ready` frame to already-ready browser clients. `ready` frames carry the current upstream epoch when known, so a WS-only client learns the boot identity before any resync and can echo `epoch` on its next resume; resync frames carry the same field. The browser treats this as a reconnect edge and can run scoped state repair without requiring the browser WS to close. Hub `restart` statuses (epoch change or upstream resync control) additionally send every ready client a resync control with the reconnectable tail.
