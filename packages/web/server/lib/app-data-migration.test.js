...
describe('migrateLegacyAppDataDir', () => {
  ...
  beforeEach(() => {
    ...
    previousDataDir = process.env.OMPCHAMBER_DATA_DIR;
  ...
  it('never touches the home layout when OMPCHAMBER_DATA_DIR is set', async () => {
    writeSettings(legacyDir(), { uiPassword: 'secret' });
    process.env.OMPCHAMBER_DATA_DIR = path.join(tempHome, 'isolated-data');
    const migrate = await loadMigration();
