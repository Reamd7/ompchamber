...
describe('callSmallModel — custom provider config', () => {
  ...
  describe('config-supplied credentials (no auth.json entry)', () => {
    ...
    it('sends configured provider headers alongside the bearer token', async () => {
      process.env.OMPCHAMBER_TEST_GATEWAY_KEY = 'sub-key';
      readConfig.mockReturnValue({
        provider: {
          custom: {
            options: {
    ...
    it('resolves a relative header file from the config layer that defines it', async () => {
      ...
      vi.spyOn(fs, 'readFileSync').mockImplementation((filePath) => {
        // path.resolve() normalizes the comparison: on Windows the resolver
        // produces a drive-anchored path (C:\config\gateway-key) for the same
        // POSIX-style config path this test writes.
        if (path.resolve(String(filePath)) === path.resolve(secretPath)) return 'sub-key\n';
      ...
      expect(fs.readFileSync).toHaveBeenCalledWith(path.resolve(secretPath), 'utf8');
    });
