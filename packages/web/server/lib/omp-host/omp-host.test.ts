...
describe('projection', () => {
  ...
  test('toolFinished passes structured metadata through the transient tool part', () => {
    ...
    const part = emitted
      ...
      .at(-1);
    // SAFETY: test fixture narrowing — the asserted shape is the harness contract this test reads.
    expect((part as { state: { status?: string } }).state.status).toBe('completed');
    // SAFETY: test fixture narrowing — the asserted shape is the harness contract this test reads.
    expect((part as { state: { output?: string } }).state.output).toBe('ans');
    // SAFETY: test fixture narrowing — the asserted shape is the harness contract this test reads.
  ...
  test('streaming projector emits matching part ids for the final projection', () => {
    ...
    // SAFETY: test fixture narrowing — the asserted shape is the harness contract this test reads.
    const finalPartIds = partEvents.slice(-2).map((e) => (e.properties as import('./projection.ts').WirePartUpdatedProperties).part.id);
    ...
    // SAFETY: test fixture narrowing — the asserted shape is the harness contract this test reads.
    const partOf = (e: EmittedEvent) => (e.properties as import('./projection.ts').WirePartUpdatedProperties).part;
    const finalSnapshotIds = partEvents
      .filter((e) => { const p = partOf(e); return p.state?.status === 'completed' || p.type === 'text'; })
      .map((e) => partOf(e).id);
    expect(finalSnapshotIds.length).toBeGreaterThan(0);

    const deltas = emitted.filter((e) => e.type === 'message.part.delta');
    // SAFETY: test fixture narrowing — the asserted shape is the harness contract this test reads.
    expect(deltas.map((d) => (d.properties as import('./projection.ts').WirePartDeltaProperties).delta).join('')).toBe('partial+more');
    // SAFETY: test fixture narrowing — the asserted shape is the harness contract this test reads.
    expect(deltas.every((d) => (d.properties as import('./projection.ts').WirePartDeltaProperties).field === 'text')).toBe(true);
  });
