...
import { RingEventBus, WireEventBus, OmpEventBus } from './events.js';
import { projectDividerMessage, projectConversation } from './projection.js';
...
describe('registerEndpoints mounting smoke (wiring regression guard)', () => {
  test('mounts every route group against a stub engine without throwing', async () => {
    ...
    const stubEngine = {
      ...
      dialogs: { mount: (r) => { r('GET', '/omp/dialogs-stub', async () => ({})); } },
      modesDomain: {},
      uriDomain: { mount: (r) => { r('GET', '/omp/uri-stub', async () => ({})); } },
