...
describe('buildModelsPayload (01 §5.3(1))', () => {
  test('roles snapshot, cycleOrder, enabledModels, fallbackChains, legacyDefaults', async () => {
    ...
    try {
      ...
      // SAFETY: Effort is a const enum — its string literal is not directly
      // assignable to the nominal enum type, but the wire value IS the literal.
      expect(payload.defaultThinkingLevel).toBe('high' as SettingValue<'defaultThinkingLevel'>);
...
describe('PUT validation (06 §5.3.1)', () => {
  ...
  test('invalid scope / body shapes fail fast', async () => {
    ...
    try {
      ...
      // Deliberately malformed wire body: applySettingsChanges' runtime
      // typeof guard is the assertion target, so bridge past the input type
      // with a JSON-parsed payload that keeps the invalid shape.
      // SAFETY: parse-time assertion — `{ changes: "nope" }` must reach the
      // runtime guard untouched (same object JSON.stringify would emit).
      const malformedBody = JSON.parse('{"changes":"nope"}') as SettingsChangesInput;
      expect((await applySettingsChanges(store, malformedBody)).body.error).toBe('invalid-body');
