...
export class OmpHostEngine {
  ...
  constructor({ agentDir } = {}) {
    ...
    this.uriDomain = createUriDomain({
      ...
      localOptionsFor: (sessionId, directoryKey) => createLocalProtocolOptions(sessionId, directoryKey, this.#sessionDirFor(directoryKey)),
  ...
  async #materializeNow(sessionId, directoryKey) {
    ...
    const { session, setToolUIContext } = await createAgentSession({
      ...
      localProtocolOptions: createLocalProtocolOptions(sessionId, directoryKey, this.#sessionDirFor(directoryKey)),
      ...(model ? { model } : {}),
