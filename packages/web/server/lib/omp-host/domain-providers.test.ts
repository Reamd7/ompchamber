...
describe('GET /omp/providers (listOmpProviders)', () => {
  test('tags file providers with config + masked key; engine-only providers tagged engine', async () => {
    ...
    const byId = Object.fromEntries(result.providers.map((p) => {
      // SAFETY: alpha — the only row whose file-arm fields are read below
      // (baseUrl/hasApiKey) — is asserted `source: 'file'` first, so the
      // one-time widen to ListedProviderRow is sound.
      return [p.id, p as ListedProviderRow];
    }));
...
describe('PUT /omp/providers (putOmpProvider)', () => {
  ...
  test('dialog null payload clears per-model baseUrl instead of writing literal nulls (engine drops whole models.yml on null)', async () => {
    ...
    const parsed = parseDocument(finalFile, aliasLimitOptions).toJS(aliasLimitOptions);
  ...
  test('dialog un-check round trip: explicit reasoning false lands, omitMax null clears, projection surfaces both', async () => {
    ...
    const { ModelsConfigFile } = await import('@oh-my-pi/pi-coding-agent/config/models-config');
    const { parseDocument } = await import('yaml');
    const value = parseDocument(readFileSync(modelsPath, 'utf8'), aliasLimitOptions).toJS(aliasLimitOptions);
  ...
  test('rejects invalid payloads without touching the file', async () => {
    ...
    const cases: Array<[PutOmpProviderInput, string]> = [
    ...
    const result = await fetchOmpProviderModels(
      { id: 'alpha' },
      {
        ...
        fetchImpl: async (url, init) => {
          // SAFETY: fetchOmpProviderModels builds the fetch init inline
          // with a plain header record (HeadersInit's record arm), so the
          // Authorization read below lands on that record.
