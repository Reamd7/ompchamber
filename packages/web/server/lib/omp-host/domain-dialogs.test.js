...
import {
  ...
  ORPHAN_WINDOW_MS,
  PRESENT_TTL_MS,
} from './domain-dialogs.js';
import { OmpEventBus } from './events.js';
...
describe('createDialogBridge', () => {
  ...
  test('notify forwards to the hook; terminal-only members are inert no-ops', async () => {
    ...
    bridge.setFooter(() => {});
    bridge.setHeader(undefined);
    bridge.setTitle('t');
    expect(await bridge.custom(() => {})).toBeUndefined();
    bridge.setEditorText('x');
    bridge.pasteToEditor('y');
    expect(bridge.getEditorText()).toBe('');
    bridge.addAutocompleteProvider(() => {});
    bridge.setEditorComponent(undefined);
    const unsubscribe = bridge.onTerminalInput(() => {});
...
describe('registerDialogEndpoints', () => {
  const mountRoutes = (feature) => {
    ...
    const call = async (key, { params = {}, body, url } = {}) => {
