...
Uma integração é um pequeno plugin que adiciona um provedor ao OMPChamber usando uma assinatura que você já tem. Você as gerencia em **Settings → Integrations**.
...
O Cursor torna disponíveis no OMPChamber os modelos incluídos no seu plano Cursor.
