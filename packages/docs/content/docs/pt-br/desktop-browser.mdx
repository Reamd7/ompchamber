...
**Limpar cookies** e **Limpar dados em cache** valem só para este painel. Sua sessão do OMPChamber e as outras janelas ficam intactas.
...
Ele também pode salvar uma imagem do que está vendo em `.ompchamber/screenshots/` no seu projeto e mostrá-la na resposta. É isso que torna possível um antes e depois, e o arquivo continua lá para anexar a um pull request.

As ações do navegador são a **ferramenta OMPChamber Web**, que pode ser ligada e desligada por conta própria em **Configurações → Geral → Ferramentas do OMPChamber**.
