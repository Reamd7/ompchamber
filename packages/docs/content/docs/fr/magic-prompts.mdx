...
Certains prompts ont une partie visible (le message que vous verriez) et une partie instructions (des consignes cachées pour l’agent). Les prompts peuvent contenir des `{{placeholders}}` qu’OMPChamber remplit, comme le diff ou le titre de l’issue — gardez-les en place.
