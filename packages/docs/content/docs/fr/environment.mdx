...
Remplace le dossier de données OpenChamber. La valeur par défaut est `~/.config/ompchamber`.
...
Autre exécutable Git de remplacement. Préférez `OMPCHAMBER_GIT_BINARY` pour une configuration propre à OpenChamber.
