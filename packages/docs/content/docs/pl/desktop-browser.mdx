...
**Wyczyść ciasteczka** i **Wyczyść dane w pamięci podręcznej** dotyczą wyłącznie tego panelu. Twoja sesja OMPChamber i pozostałe okna zostają nietknięte.
...
Może też zapisać obraz tego, co widzi, w `.ompchamber/screenshots/` w twoim projekcie i pokazać go w odpowiedzi. To właśnie umożliwia porównanie przed i po, a plik zostaje, by dołączyć go do pull requesta.

Działania w przeglądarce to **narzędzie OMPChamber Web**, które włącza się i wyłącza osobno w **Ustawienia → Ogólne → Narzędzia OMPChamber**.
