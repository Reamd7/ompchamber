...
**Clear cookies** and **Clear cached data** apply to this panel alone. Your OMPChamber session and any other window are untouched.
...
It can also save a picture of what it is looking at into `.ompchamber/screenshots/` in your project and show it to you in its reply. That is what makes a before-and-after possible, and the file stays there afterwards to attach to a pull request.

The browser actions are the **OMPChamber Web tool**, which can be turned on and off on its own in **Settings → General → OMPChamber Tools**.
