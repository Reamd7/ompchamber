...
// Directory activation re-reads the OMPChamber defaults, which are global,
...
const OMPCHAMBER_DEFAULTS_FRESH_MS = 15_000;
let ompChamberDefaultsCache: { at: number; request: Promise<OMPChamberDefaults> } | null = null;

const fetchOMPChamberDefaults = (): Promise<OMPChamberDefaults> => {
    const now = Date.now();
    if (ompChamberDefaultsCache && now - ompChamberDefaultsCache.at < OMPCHAMBER_DEFAULTS_FRESH_MS) {
        return ompChamberDefaultsCache.request;
    }
    const request = requestOMPChamberDefaults();
    ompChamberDefaultsCache = { at: now, request };
    request.catch(() => {
        if (ompChamberDefaultsCache?.request === request) ompChamberDefaultsCache = null;
    });
    return request;
};

const requestOMPChamberDefaults = async (): Promise<OMPChamberDefaults> => {
...
interface ConfigStore {
    ...
    applyDefaultModelAgentSelection: (options?: { projectDefaultModel?: string; projectDefaultVariant?: string; ompDefaultModel?: { providerId: string; modelId: string } }) => void;
