  ...
  for (const [rawDirectory, rawState] of Object.entries(source)) {
    ...
    if (candidate.widthByMode && typeof candidate.widthByMode === 'object') {
      for (const [mode, value] of Object.entries(candidate.widthByMode as Record<string, unknown>)) {
        if (
          (mode === 'diff' || mode === 'file' || mode === 'context' || mode === 'plan' || mode === 'chat' || mode === 'browser' || mode === 'git' || mode === 'pr' || mode === 'linear' || mode === 'notes' || mode === 'terminal' || mode === 'timeline')
          && typeof value === 'number'
    ...
    persist(
      ...
      }),
      {
        ...
        migrate: (persistedState, version) => {
          ...
          // v15/16 -> v16: the main-area surface concept is gone from persistence
          ...
          // value cannot rehydrate into a blank main area. Fork snapshots that
          // already reached v16 still carry the renamed pair, so the cleanup
          // runs for every version below v17.
          if (version < 17) {
