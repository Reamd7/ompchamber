...
- The server also fs-watches registered projects' `.git/worktrees` metadata and pushes `worktrees-changed` events; `packages/ui/src/lib/worktrees/worktreeEventSync.ts` (mounted via `SyncAppEffects`) refreshes only the affected projects in the shared worktree map, so worktrees created or removed by any external source appear without a session event or reload. Failed listings keep last-known worktrees.
...
- A `database-2` glyph marks sessions the omp host keeps resident: the wire `live` field (`materializing`/`live`/`evicting`/`failed`, colored by state) plus a transcript-size estimate tooltip. It shares the trailing-metadata slot with goal/branch markers and follows the same hover-fade rule; the memo comparator watches `live`/`transcriptBytes` so the host's post-eviction `session.updated` repaints the row.
