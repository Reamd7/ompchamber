...
describe('projectSidebarCollection', () => {
  ...
  test('keeps managed Chats in a dedicated projection and out of project and Recent ownership', () => {
    const managed = session('managed', '/home/.config/ompchamber/chats/2026-08-24/session-managed');
  ...
  test('excludes a /btw fork before project ownership and restores it when the marker is removed', () => {
    const fork = {
      ...session('fork', '/home/.config/ompchamber/chats/2026-08-24/session-fork'),
      metadata: { ompchamber: { kind: 'btw', originalSessionID: 'parent' } },
    };
    const project = session('project', '/workspace/a');
    const input = {
      liveSessions: [],
  ...
  test('keeps a ranked managed root and its active child in the Chats hierarchy', () => {
    const managedRoot = { ...session('managed-root', '/home/.config/ompchamber/chats/2026-08-24/session-root'), time: { created: 1, updated: 1 } };
    const managedChild = {
      ...session('managed-child', '/home/.config/ompchamber/chats/2026-08-24/session-root'),
