...
import { StrippedToolCallsLine } from './message/StrippedToolCallsLine';
...
const EDIT_TOOL_NAMES = new Set(['apply_patch', 'edit', 'write', 'multiedit', 'str_replace', 'str_replace_based_edit_tool', 'create', 'file_write']);
...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, previousMessage, nextMessage, onContentChange, animationHandlers, turnGroupingContext, assistantHeaderMessageId, isInActiveTurn = false, activeStreamingPhase = null, animateUserOnMount = false, onUserAnimationConsumed, reviewTransferDirection = null }) => {
  ...
  const previousUserMetadata = React.useMemo(() => {
    ...
    const resolvedAgent = typeof mode === 'string' && mode.trim().length > 0 ? mode : typeof agent === 'string' && agent.trim().length > 0 ? agent : undefined;
  ...
  const previousIsModeSwitchMessage = React.useMemo(() => {
    ...
    for (let i = 0; i < parts.length; i++) {
      const part = parts[i] as unknown as {
        type?: string;
        text?: string;
        synthetic?: boolean;
      };
  ...
  const messageSessionDirectory = useSessionUIStore(React.useCallback((state) => (sessionId ? state.getDirectoryForSession(sessionId) : null), [sessionId]));
  ...
  const contextModelSelection = React.useMemo(() => {
    ...
    if (ompRolesOn) {
      if (ompSessionModel?.provider && ompSessionModel?.id) {
        return {
          providerId: ompSessionModel.provider,
          modelId: ompSessionModel.id
        };
      }
      return null;
    }
  ...
  // Branch/fork history rewrite count (wire info.metadata.ompStrippedToolCalls,
  // omp-host projection.js): renders the elided-activity line.
  const strippedToolCalls = React.useMemo(() => {
    const metadata: unknown = 'metadata' in message.info ? message.info.metadata : undefined;
    if (typeof metadata !== 'object' || metadata === null || !('ompStrippedToolCalls' in metadata)) return null;
    const value: unknown = metadata.ompStrippedToolCalls;
  ...
  const modelHasVariants = React.useMemo(() => {
    ...
    const model = provider.models.find((m: Record<string, unknown>) => (m as Record<string, unknown>).id === modelID) as { variants?: Record<string, unknown> } | undefined;
  ...
  const fallbackMarkerActive = Boolean(!isUser && ompFallback?.active && messageCompletedAt !== null && messageCompletedAt >= ompFallback.updatedAt);
  ...
  const isPinnedIntoContext = useGlobalSessionsStore((state) => {
    const session = state.activeSessions.find((candidate) => candidate.id === sessionId) ?? state.archivedSessions.find((candidate) => candidate.id === sessionId);
  ...
  // keeps the plain-text look. User-role messages qualify only when the
  // projection stamped them as collapsed developer notes, so real user
  // bubbles never reroute through the tier renderer.
  const ompCustomMessage = React.useMemo(() => {
    if (isUser) {
      const metadata: unknown = 'metadata' in message.info ? message.info.metadata : undefined;
      const ompRole = typeof metadata === 'object' && metadata !== null && 'ompRole' in metadata && typeof metadata.ompRole === 'string' ? metadata.ompRole : null;
      return ompRole === 'developer' ? parseOmpCustomMessage(message.info, visibleParts) : null;
    }
    return parseOmpCustomMessage(message.info, visibleParts);
  ...
  const agentMention = React.useMemo(() => {
    ...
    const partWithName = mentionPart as {
      name?: string;
      source?: { value?: string };
    };
    ...
    const rawValue = partWithName.source && typeof partWithName.source.value === 'string' && partWithName.source.value.trim().length > 0 ? partWithName.source.value : `@${name}`;
  ...
  const previousIsHiddenUserMessage = React.useMemo(() => !isUser && isHiddenUserMessage(previousMessage, { planModeEnabled }), [isUser, planModeEnabled, previousMessage]);

  const nextIsHiddenUserMessage = React.useMemo(() => !isUser && isHiddenUserMessage(nextMessage, { planModeEnabled }), [isUser, planModeEnabled, nextMessage]);
  ...
  const assistantError = React.useMemo(() => {
    ...
    const errorInfo = (message.info as { error?: unknown } | undefined)?.error as { data?: { message?: unknown }; message?: unknown; name?: unknown } | undefined;
    if (!errorInfo) {
      return undefined;
    }
  ...
  const assistantTopPaddingClass = !isUser && shouldShowHeader && !previousIsHiddenUserMessage ? (stickyUserHeader ? (isMobile ? 'pt-4' : 'pt-6') : 'pt-0') : 'pt-0';
  const userMessageRadius = 'var(--radius-xl)';

  return (
    <>
      <div className={cn('group w-full', isUser ? (isMobile ? 'pt-2' : 'pt-4') : assistantTopPaddingClass, isUser ? 'pb-0' : isFollowedByAssistant || nextIsHiddenUserMessage ? 'pb-0' : 'pb-2')} id={`message-${message.info.id}`} data-message-id={message.info.id} ref={messageContainerRef}>
        <div className="chat-message-column relative">
          {isUser ? (
            ompCustomMessage ? (
              <OmpCustomMessage data={ompCustomMessage} />
            ) : displayParts.length === 0 ? null : (
              <FadeInOnReveal forceAnimation skipAnimation={!animateUserOnMount} ignoreContextDisabled respectReducedMotion>
            ...
            <div className="relative">
              {turnGroupingContext?.cacheMiss && turnGroupingContext.isFirstAssistantInTurn ? <CacheMissDivider reprocessedTokens={turnGroupingContext.cacheMiss.reprocessedTokens} /> : null}
              {supersededByRetry ? (
                <div className="flex max-w-full items-center gap-1.5 pb-1.5 text-xs text-muted-foreground/80" data-retry-superseded={message.info.id}>
              ...
              {!ompCustomMessage && isLastAssistantInTurn && isMessageCompleted ? <TurnUsageRow sessionId={message.info.sessionID} messageId={message.info.id} wireInfo={message.info} /> : null}
            </div>
          )}
        </div>
      </div>
