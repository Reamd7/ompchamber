...
import type { Message, Part, Session } from '@/lib/opencode/wire'
...
const ChatViewport = React.memo(({
    ...
    ), [currentSessionId, isLoadingOlder, onLoadOlder, showLoadOlderButton, t]);
...
export const ChatContainer: React.FC<ChatContainerProps> = ({
    ...
    React.useEffect(() => {
        ...
        const scopedWindow = window as typeof window & {
            __ompchamberApplyChatSettingsSync?: (payload: { allowPromptingSubagentSessions: boolean }) => void;
        };
        ...
        scopedWindow.__ompchamberApplyChatSettingsSync = applySync;
        window.addEventListener('message', handleMessage);
        parentWindow.postMessage({ type: 'ompchamber:chat-settings-request' }, window.location.origin);
        return () => {
            window.removeEventListener('message', handleMessage);
            if (scopedWindow.__ompchamberApplyChatSettingsSync === applySync) {
                delete scopedWindow.__ompchamberApplyChatSettingsSync;
            }
        };
    }, []);
    ...
    React.useEffect(() => {
        // Race guard for cold deep links: this effect can fire before the
        // router has applied ?session=… — auto-opening a draft then would
        // blank the transcript until the user clicks away. Never auto-open
        // while the URL still names a session.
        const urlNamesSession = typeof window !== 'undefined'
            && Boolean(new URLSearchParams(window.location.search).get('session'));
        if (autoOpenDraft && !liveSessionId && !draftOpen && !urlNamesSession) {
	...
	return (
        ...
        {/* omp engine surfaces (fork): dialogs, the internal URI viewer, and
            plan review overlay — mounted once here instead of per render
            branch. Skipped for the un-materialized draft exactly where the
            fork's former draft early-return had no layers. */}
        {currentSessionId || !draftOpen ? (
            <>
                <OmpDialogLayer directory={effectiveSessionDirectory} sessionId={currentSessionId ?? undefined} />
                <InternalUriViewer directory={effectiveSessionDirectory} sessionId={currentSessionId ?? undefined} />
                <OmpPlanReviewOverlay directory={effectiveSessionDirectory} sessionId={currentSessionId ?? undefined} />
            </>
        ) : null}
        </>
    );
