import MarkdownShikiWorker from './markdown-shiki.worker.ts?worker&inline';

// Inline worker (matching lib/diff/workerFactory): the Shiki/Oniguruma module
// graph is embedded as a blob, so the worker never depends on runtime
// chunk-URL resolution — which is also what keeps it loadable from VS Code
// webviews.
import {
  contentFingerprint,
...
const createWorker = (): Worker | undefined => {
  ...
  try {
    const instance = new MarkdownShikiWorker();
    worker = instance;
