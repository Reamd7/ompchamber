...
export const KeyboardShortcutsSettings: React.FC = () => {
  ...
  const shortcutDisplay = (action: CustomizableShortcutAction): string => {
    ...
    const formatted = formatShortcutForDisplay(
      combo,
      t('settings.ompchamber.keyboardShortcuts.unassigned'),
    );
    if (!isPrefixStyle || !combo || combo === UNASSIGNED_SHORTCUT) return formatted;
    const suffix = action.id === 'switch_session_tab'
      ? t('settings.ompchamber.keyboardShortcuts.action.switch_session_tab.suffix')
      : t('settings.ompchamber.keyboardShortcuts.action.switch_context_surface.suffix');
      ...
      {CATEGORIES.map((category, categoryIndex) => {
        ...
        return (
          <SettingsSection
            ...
            title={t(`settings.ompchamber.keyboardShortcuts.category.${category}`)}
            divider={categoryIndex !== 0}
            info={categoryIndex === 0 ? t('settings.ompchamber.keyboardShortcuts.tooltip') : undefined}
            headerAction={categoryIndex === 0 ? (
              <Button type="button" variant="outline" size="xs" className="!font-normal" onClick={() => {
                resetAllShortcutOverrides();
                persist({});
              }}>
                {t('settings.ompchamber.keyboardShortcuts.actions.resetAll')}
              </Button>
            ) : undefined}
          >
            <div className="space-y-2">
              {categoryActions.map((action) => (
                <SettingsFieldRow key={action.id} label={t(action.settingsLabelKey)}>
                  ...
                  >
                    {t('settings.ompchamber.keyboardShortcuts.actions.edit')}
                  </Button>
