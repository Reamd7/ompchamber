...
export const PasskeySettings: React.FC = () => {
  ...
  const loadPasskeys = React.useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : t('settings.ompchamber.passkeys.toast.loadFailed');
  ...
  const handleRegisterPasskey = React.useCallback(async () => {
    if (!status.enabled) {
      const message = t('settings.ompchamber.passkeys.toast.enableUiPasswordFirst');
      setErrorMessage(message);
    ...
    try {
      ...
      await loadPasskeys();
      toast.success(t('settings.ompchamber.passkeys.toast.added'));
    } catch (error) {
      if (isPasskeyCeremonyAbort(error)) {
        toast.message(t('settings.ompchamber.passkeys.toast.setupCanceled'));
        return;
      }

      const message = error instanceof Error ? error.message : t('settings.ompchamber.passkeys.toast.addFailed');
      setErrorMessage(message);
      toast.error(message);
    } finally {
      setIsRegistering(false);
  ...
  const handleRevokePasskey = React.useCallback(async (id: string) => {
    ...
    try {
      ...
      setStatus(await fetchPasskeyStatus());
      await loadPasskeys();
      toast.success(t('settings.ompchamber.passkeys.toast.removed'));
    } catch (error) {
      const message = error instanceof Error ? error.message : t('settings.ompchamber.passkeys.toast.removeFailed');
      setErrorMessage(message);
      toast.error(message);
    } finally {
      setRevokingId(null);
  ...
  const handleResetAllAuth = React.useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : t('settings.ompchamber.passkeys.toast.clearAuthFailed');
      setErrorMessage(message);
      toast.error(message);
  ...
  return (
    <SettingsSection title={t('settings.ompchamber.passkeys.title')}>
      <div className="space-y-2">
        <SettingsFieldRow label={t('settings.ompchamber.passkeys.field.currentDevice')}>
          <Button
            type="button"
          ...
          >
            {isRegistering ? t('settings.ompchamber.passkeys.actions.cancelSetup') : t('settings.ompchamber.passkeys.actions.add')}
          </Button>
          <Button
            type="button"
            variant="ghost"
          ...
          >
            {isResetting ? t('settings.ompchamber.passkeys.actions.signingOut') : t('settings.ompchamber.passkeys.actions.signOutEverywhere')}
          </Button>
        </SettingsFieldRow>

        {!status.enabled && (
          <p className="typography-meta text-muted-foreground">
            {t('settings.ompchamber.passkeys.state.uiPasswordRequired')}
          </p>
        )}
        ...
        {isLoading ? (
          <p className="typography-meta text-muted-foreground">{t('settings.ompchamber.passkeys.state.loading')}</p>
        ) : passkeys.length === 0 ? (
          <p className="typography-meta text-muted-foreground">{t('settings.ompchamber.passkeys.state.noneSaved')}</p>
        ) : (
            ...
            {passkeys.map((passkey) => (
              ...
              >
                <span className="typography-meta text-muted-foreground truncate">
                  {passkey.lastUsedAt
                    ? t('settings.ompchamber.passkeys.item.lastUsed', {
                        time: formatTimestamp(passkey.lastUsedAt, t('settings.ompchamber.passkeys.time.neverUsed'), timeFormatPreference),
                      })
                    : t('settings.ompchamber.passkeys.item.added', {
                        time: formatTimestamp(passkey.createdAt, t('settings.ompchamber.passkeys.time.neverUsed'), timeFormatPreference),
                      })}
                </span>
                <Button
                  type="button"
                  ...
                  className="!font-normal text-muted-foreground hover:text-foreground"
                >
                  {revokingId === passkey.id ? t('settings.ompchamber.passkeys.actions.removing') : t('settings.common.actions.delete')}
                </Button>
              </SettingsFieldRow>
            ))}
          </div>
