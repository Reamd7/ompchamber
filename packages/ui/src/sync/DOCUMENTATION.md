...
9. When an ambiguous failure still ends in rollback (the confirmation refetch could not prove acceptance — it failed, or its newest-30 window no longer contains the user row because the accepted turn already streamed more messages), the rollback additionally invalidates the loader's coverage for that session and forces an authoritative reload. The engine streams assistant rows anchored by `parentID` to the user row the rollback removed, and the timeline drops rows whose anchor is missing, so without the forced reload the transcript stays blank until some later cold page load happens to restore the anchor. A definite failure keeps the plain rollback: the server provably rejected the prompt, so the pre-send snapshot remains authoritative.
