...
The pipeline's transport layer carries the stream-continuity contract (docs/plan.md §5.2/§5.3.1). `omp.stream.resync` controls (SSE data-less frames or WS `resync` frames) re-base the resume cursor onto the server-provided tail — `0`/empty clears it — and fire `onResync`, which reconciles every mounted directory from HTTP before deltas are trusted again; `omp.stream.boot` frames only teach the boot epoch that is echoed back as `x-omp-epoch` so same-process resumes skip the forced reconcile. Locally, retention is bounded: at most 64 tracked directories, 4096 queued events or 8 MiB of estimated serialized bytes per directory (byte figures are UTF-16 estimates, not heap measures). Overflow drops that directory's queued window, fires `onResync("queue-overflow", directory)` — scoped, so only that directory reconciles — and re-seeds the queue from the incoming event instead of buffering without bound; `pipeline.stats()` exposes the drop/overflow counters. Cleanup clears every timer, queue, and coalescing map.
...
8. A prompt send that fails **after** the request left the client is ambiguous, never a definite failure: the server may already be answering it. Transports tag those errors (`markAmbiguousTransportFailure` in `@/lib/relay/transport-error`; the relay tunnel tags every stream that dies with a request in flight), and `isAmbiguousSendFailure` reads the tag before falling back to status/text heuristics. An ambiguous failure waits for the connection to return, refetches messages, and confirms the optimistic message in place instead of rolling it back — rolling it back lets the message queue re-send a prompt the engine is already running, producing two independent AI responses for one user message. The refetch walks history, not just the newest page: each page without the row hands its `x-next-cursor` to the next fetch, because a long turn can push the accepted user row past the newest window before the send response fails.
9. When an ambiguous failure still ends in rollback (the walk exhausted history or its bounded attempt budget without the row, or every request failed), the rollback additionally invalidates the loader's coverage for that session and forces an authoritative reload. The engine streams assistant rows anchored by `parentID` to the user row the rollback removed, and the timeline drops rows whose anchor is missing, so without the forced reload the transcript stays blank until some later cold page load happens to restore the anchor. A definite failure keeps the plain rollback: the server provably rejected the prompt, so the pre-send snapshot remains authoritative.
10. `SessionLiveActivity` has three answers and `unknown` is never `idle`. `getSessionLiveActivity` reports `active` when any child store or the global session-status index holds a non-idle status, `idle` only when a child store actually covers the session's directory, and `unknown` otherwise — child stores are evicted for background directories, and the global index keeps only non-idle entries, so absence of a status is not proof of idleness. Callers that gate a destructive action (worktree moves) must refuse on `unknown`.
11. Revert and unrevert cascade through known descendant sessions before mutating the parent. Revert uses the first descendant user message at or after the parent's target timestamp, including equal timestamps because message IDs do not define chronology. A descendant failure is logged and does not block its siblings or the parent. The parent runs last so its shared-directory file snapshot remains authoritative. A busy descendant is aborted before it is reverted, like the parent, so nothing keeps writing past the revert boundary. Redo clears the revert marker on every descendant, including markers the user set on a subagent independently of the parent undo.
12. Shell-mode sends (`inputMode: "shell"`, the composer's `!` prefix) are an omp-native capability, not wire `session.shell` (which the omp host intentionally answers 501 — OpenCode's model-mediated shell has no omp analog). `routeMessage` calls the registered runtime API `ompSession.runBash`, which POSTs the OpenChamber-owned `/api/omp/sessions/{id}/bash` route; the engine runs the command through the session's own BashRunner and streams the `[omp:bash]` card over the normal wire events. A residual leading `!` in the submitted text is the `!!` marker: it sets `excludeFromContext` (the run stays out of model context) and is stripped before dispatch. A runtime switch mid-send rejects rather than re-resolving it against the new runtime, and a host without `bash.…
