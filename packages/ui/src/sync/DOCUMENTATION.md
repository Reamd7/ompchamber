...
11. Shell-mode sends (`inputMode: "shell"`, the composer's `!` prefix) are an omp-native capability, not wire `session.shell` (which the omp host intentionally answers 501 — OpenCode's model-mediated shell has no omp analog). `routeMessage` calls the registered runtime API `ompSession.runBash`, which POSTs the OpenChamber-owned `/api/omp/sessions/{id}/bash` route; the engine runs the command through the session's own BashRunner and streams the `[omp:bash]` card over the normal wire events. A residual leading `!` in the submitted text is the `!!` marker: it sets `excludeFromContext` (the run stays out of model context) and is stripped before dispatch. A runtime switch mid-send rejects rather than re-resolving against the new runtime, and a host without `bash.v1` surfaces as a clear not-supported error. The projected row is `role:'user'` with a synthetic `shellAction` text part — `normalizeUserDisplayParts` keeps it visible by that payload, and `SessionErrorNotice` exempts `metadata.ompRole` `bash`/`python` rows from the unanswered-message watchdog (no reply is ever expected).
