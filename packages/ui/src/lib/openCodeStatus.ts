...
export const buildOpenCodeStatusReport = async (): Promise<string> => {
  ...
  const openChamberHealth: OMPChamberHealthSnapshot | null = await (async () => {
  ...
  const isDesktop = origin.startsWith('ompchamber-ui://');
  lines.push('');
  ...
  if (isDesktop) {
    ...
    lines.push(`- OMPChamber desktop: ${isWindows
      ? '%APPDATA%\\OMPChamber\\logs\\main.log'
      : isMacDesktop
        ? '~/Library/Logs/OMPChamber/main.log'
        : '~/.config/OMPChamber/logs/main.log'}`);
  }
