...
const CHANGELOG_URL = 'https://raw.githubusercontent.com/Reamd7/ompchamber/main/CHANGELOG.md';
const GITHUB_BUG_REPORT_URL = 'https://github.com/Reamd7/ompchamber/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/Reamd7/ompchamber/issues/new?template=feature_request.yml';
...
const requestQuitWithConfirmation = async () => {
  ...
  try {
    const result = await dialog.showMessageBox({
      type: 'warning',
      title: 'Quit OMPChamber?',
      message: 'Quit OMPChamber?',
...
// session expires) reuses the same "OMPChamber Desktop" record instead of
...
const maybeShowNativeNotification = (rawInput) => {
  ...
  notification.on('click', () => {
    ...
    if (sessionId) {
      emitToPrimaryWindow('ompchamber:open-session', { sessionId, directory });
    }
    release();
  });
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="OMPChamber loading icon">
        <!-- Official Oh My Pi mark: π bar + two legs + orange plugin connector -->
        <g transform="translate(-2.8 10.4) scale(0.88)">
          <rect x="10" y="8" width="100" height="12" rx="2" fill="var(--splash-logo-fill)"/>
          <rect x="25" y="20" width="12" height="62" rx="2" fill="var(--splash-logo-fill)"/>
          <rect x="75" y="20" width="12" height="45" rx="2" fill="var(--splash-logo-fill)"/>
...
const parseConnectPairingDeepLinkPayload = (raw) => {
  ...
  try {
    ...
    return {
      pairingId,
      secret,
      label: typeof payload.label === 'string' && payload.label.trim() ? payload.label.trim() : 'OMPChamber',
...
const dispatchDeepLink = (link) => {
  ...
  if (link.type === 'session' && link.value) {
    emitToPrimaryWindow('ompchamber:open-session', { sessionId: link.value });
    return;
  }
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    case 'desktop_read_file': {
      ...
      const DENIED_SEGMENTS = ['.ssh', '.aws', '.gnupg', '.gpg', '.config/gh', '.config/ompchamber/credentials'];
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires OMPChamber.app to be installed in /Applications');
          }
        } catch (error) {
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About OMPChamber', click: () => dispatchAction('about') },
        {
